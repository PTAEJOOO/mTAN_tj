...
def split_and_patch_batch(data_dict, args, n_observed_tp, patch_indices):
	...
	n_batch, n_tp, n_dim = observed_data.shape # (bs, n_observed_tp, D)
	...
	observed_mask_patches = observed_mask.view(n_batch, 1, n_tp, n_dim).repeat(1, args.npatch, 1, 1) # n_batch, npacth, n_tp, n_dim
	...
	for i in range(args.npatch):
		...
		n_data_points = observed_mask[:, st_ind:ed_ind+1].sum(dim=1).max().item() # observed_mask[:, st_ind:ed_ind+1].sum(dim=1) : [n_batch, D]
