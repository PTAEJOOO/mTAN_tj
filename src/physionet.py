...
def patch_variable_time_collate_fn(batch, args, device = torch.device("cpu"), data_type = "train", 
	...
	n_observed_tp = torch.lt(combined_tt, args.history).sum() # batch 안에서 unique time points 수
	observed_tp = combined_tt[:n_observed_tp] # (n_observed_tp, )

	patch_indices = []
	st, ed = 0, args.patch_size # ex) args.patch_size = 8
	for i in range(args.npatch): # ex) args.npatch = 3
		if(i == args.npatch-1):
	...
	data_dict = {
		"data": combined_vals, # (n_batch, T_o, D), T_o: batch 안에서 unique time points 수
